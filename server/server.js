...
app.post("/register", (req, res) => {
  ...
  if (!firstname || !email || !password) {
    return res.status(400).json({ message: "Hey buddy, fill the details first!" });
  }
  try {
      ...
      database.query(QUERY, [email], async (err, result) => {
        ...
        if (result.length > 0) {
          return res.status(400).json({ message: "This account already registered, try logging in!" });
        }
        const salt = await bcryptjs.genSalt(10);
        const hashedPassword = await bcryptjs.hash(password, salt);
        const INSERT_QUERY = `
          INSERT INTO users (firstname, lastname, email, password, gender, account_status)
          VALUES (?, ?, ?, ?, ?, 'Pending')
        `;
        database.query(INSERT_QUERY, [firstname, lastname, email, hashedPassword, gender], (err, result) => {
          if (err) return res.status(500).json({ error: err.message });
